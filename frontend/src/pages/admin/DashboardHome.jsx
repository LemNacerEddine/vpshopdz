...
const DashboardHome = () => {
  ...
  // generateInvoice function - uses HTML for proper Arabic text rendering
  const generateInvoice = (order) => {
    try {
      generateHtmlInvoice(order, storeInfo, language, text);
      toast.success(language === 'ar' ? 'تم فتح الفاتورة' : 'Invoice opened');
    } catch (error) {
  ...
  const handleCancelOrder = async (orderId) => {
    ...
    try {
      ...
      toast.error(language === 'ar' ? 'خطأ في إلغاء الطلب' : 'Error canceling order');
    }
  };
        ...
        <div className="p-4">
          <div className="flex flex-col lg:flex-row lg:items-center gap-4">
            ...
            <div className="flex items-center justify-between lg:w-32">
              <div>
                <p className="font-bold text-primary text-lg">#{order.order_id}</p>
            ...
            <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    >
                      ...
                      <span className="font-medium" dir="ltr">{order.customer_phone || order.phone}</span>
                    </a>
                    <button
                      onClick={() => copyPhone(order.customer_phone || order.phone)}
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-sm text-muted-foreground">{order.shipping_address || order.address}</p>
                </div>
              </div>
            </div>
                ...
                <div className="mt-3 pt-3 border-t space-y-1 text-sm">
                  ...
                  {order.shipping_cost !== undefined && (
                    <div className="flex justify-between text-muted-foreground">
                      ...
                      <span>{parseFloat(order.shipping_cost) > 0 ? formatPrice(order.shipping_cost) : (language === 'ar' ? 'مجاني' : language === 'fr' ? 'Gratuit' : 'Free')}</span>
                    </div>
                  )}
                ...
                {lowStockProducts.map(product => (
                  <div key={product.product_id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-muted/50">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className={`text-xs font-medium ${product.stock < 10 ? 'text-red-500' : 'text-yellow-500'
                        }`}>
