...
const ProductForm = () => {
  ...
  const l = {
    ar: {
      ...
      freeShipping: 'مجاني',
      fixedPrice: 'سعر ثابت',
      fixedShippingPrice: 'سعر الشحن الثابت (دج)',
      weight: 'الوزن (كغ)',
    ...
    fr: {
      ...
      fixedShippingPrice: 'Prix de livraison fixe (DA)',
      weight: 'Poids (kg)',
    ...
    en: {
      ...
      fixedShippingPrice: 'Fixed Shipping Price (DZD)',
      weight: 'Weight (kg)',
      dimensions: 'Dimensions (cm)',
      lengthLabel: 'Length',
      widthLabel: 'Width',
  ...
  const fetchProduct = async () => {
    try {
      ...
      setFormData({
        ...
        fixed_shipping_price: product.fixed_shipping_price?.toString() || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        fixed_shipping_price: formData.shipping_type === 'fixed_price' ? parseFloat(formData.fixed_shipping_price) || 0 : null,
          ...
          <Card className="border-blue-200 dark:border-blue-800">
            <CardHeader className="pb-3">
              <CardTitle className="flex items-center gap-2 text-blue-600">
                ...
                {text.shippingSettings}
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label>{text.shippingType}</Label>
                <Select value={formData.shipping_type} onValueChange={(v) => handleChange('shipping_type', v)}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="standard">{text.standard}</SelectItem>
                    <SelectItem value="free">{text.freeShipping}</SelectItem>
                    <SelectItem value="fixed_price">{text.fixedPrice}</SelectItem>
                  </SelectContent>
                </Select>
              </div>

              {formData.shipping_type === 'fixed_price' && (
                <div className="space-y-2">
                  <Label>{text.fixedShippingPrice}</Label>
                  <Input type="number" min="0" step="0.01" value={formData.fixed_shipping_price}
                    onChange={(e) => handleChange('fixed_shipping_price', e.target.value)} placeholder="200" />
                </div>
              )}

              <div className="space-y-2">
                <Label>{text.weight}</Label>
                <Input type="number" min="0" step="0.01" value={formData.weight}
                  onChange={(e) => handleChange('weight', e.target.value)} placeholder="0.5" />
              </div>

              <div className="space-y-2">
                <Label>{text.dimensions}</Label>
                <div className="grid grid-cols-3 gap-2">
                  <Input type="number" min="0" step="0.1" value={formData.length}
                    onChange={(e) => handleChange('length', e.target.value)} placeholder={text.lengthLabel} />
                  <Input type="number" min="0" step="0.1" value={formData.width}
                    onChange={(e) => handleChange('width', e.target.value)} placeholder={text.widthLabel} />
                  <Input type="number" min="0" step="0.1" value={formData.height}
                    onChange={(e) => handleChange('height', e.target.value)} placeholder={text.heightLabel} />
                </div>
              </div>

              <div className="flex items-center justify-between">
                <Label>{text.fragile}</Label>
                <Switch checked={formData.is_fragile} onCheckedChange={(checked) => handleChange('is_fragile', checked)} />
              </div>
            </CardContent>
          </Card>
        </div>
