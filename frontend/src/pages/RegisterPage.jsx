...
const RegisterPage = () => {
  ...
  const fetchCommunes = async (wilayaName) => {
    try {
      const response = await axios.get(`${API}/communes`, { params: { wilaya_name: wilayaName } });
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (!response.data || !response.data.authUrl) {
        ...
        toast.error(text.googleRegisterError || 'فشل التسجيل عبر Google');
        setLoading(false);
        return;
      }
  ...
  const l = {
    ar: {
      ...
      googleRegisterError: 'فشل التسجيل عبر Google'
    },
    fr: {
      ...
      commune: 'Commune',
      selectCommune: 'Sélectionnez la commune',
      ...
      googleRegisterError: 'Échec de l\'inscription Google'
    },
    en: {
      ...
      googleRegisterError: 'Google registration failed'
    }
  };
          ...
          <form onSubmit={handleSubmit}>
            ...
            {formData.wilaya && (
              <div className="space-y-2 mb-4">
                ...
                <Select value={formData.commune} onValueChange={(value) => setFormData(prev => ({ ...prev, commune: value }))}>
                  <SelectTrigger>
                    <SelectValue placeholder={text.selectCommune} />
                  </SelectTrigger>
                  <SelectContent>
                    {communes.map((commune, idx) => (
                      <SelectItem key={idx} value={commune}>{commune}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
