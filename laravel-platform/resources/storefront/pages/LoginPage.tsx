...
const LoginPage: React.FC = () => {
  ...
  // Show Google error from callback redirect if any
  useEffect(() => {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      const res = await api.get(`${apiBase}/customer/auth/google`);
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="relative">
                <Mail className="absolute top-1/2 -translate-y-1/2 right-3 h-4 w-4" style={{ color: colors.mutedForeground }} />
                <input
                  type="text"
                  value={identifier}
                  ...
                  required
                  dir="ltr"
                  className="w-full h-11 pr-10 pl-4 rounded-xl border text-sm focus:outline-none focus:ring-2"
                  style={inputStyle}
                />
              </div>
            </div>
            ...
            <div>
              <label className="block text-sm font-medium mb-1.5" style={{ color: colors.cardForeground }}>كلمة المرور</label>
              <div className="relative">
                <input
                  ...
                  required
                  dir="ltr"
                  className="w-full h-11 pl-4 pr-10 rounded-xl border text-sm focus:outline-none focus:ring-2"
                  style={inputStyle}
                />
                <button
                  type="button"
