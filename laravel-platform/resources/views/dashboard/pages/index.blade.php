...
function pagesManager() {
    return {
        ...
        async loadPages() { try { const r = await fetch('/api/store/pages', { headers: { 'Accept': 'application/json' } }); this.pages = (await r.json()).data || []; } catch(e) {} this.loading = false; },
        editPage(p) { this.editMode = true; this.editId = p.id; this.form = {...p}; this.showModal = true; },
        async savePage() {
            const url = this.editMode ? `/api/store/pages/${this.editId}` : '/api/store/pages';
        ...
        async deletePage(id) { if(!confirm('حذف هذه الصفحة؟')) return; await fetch(`/api/store/pages/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content } }); await this.loadPages(); }
    }
}
</script>
@endpush
