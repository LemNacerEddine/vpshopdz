...
function couponsManager() {
    return {
        ...
        async loadCoupons() {
            try { const r = await fetch('/api/v1/dashboard/coupons', { headers: this.hdrs() }); this.coupons = (await r.json()).data || []; } catch(e) {}
            this.loading = false;
        },
        ...
        async saveCoupon() {
            this.saving = true;
            try {
                const url = this.editMode ? `/api/v1/dashboard/coupons/${this.editId}` : '/api/v1/dashboard/coupons';
                await fetch(url, { method: this.editMode ? 'PUT' : 'POST', headers: this.hdrs(), body: JSON.stringify(this.form) });
                this.showModal = false; this.resetForm(); await this.loadCoupons();
            } catch(e) {}
            this.saving = false;
        },
        async deleteCoupon(id) { if(!confirm('حذف هذا الكوبون؟')) return; await fetch(`/api/v1/dashboard/coupons/${id}`, { method: 'DELETE', headers: this.hdrs() }); await this.loadCoupons(); },
