...
function domainsManager() {
    return {
        ...
        async loadDomains() {
            try { const r = await fetch('/api/v1/dashboard/domains', { headers: { 'Accept': 'application/json' } }); this.domains = (await r.json()).data || []; } catch(e) {}
        },
        async addDomain() {
            this.saving = true;
            try { await fetch('/api/v1/dashboard/domains', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content }, body: JSON.stringify({ domain: this.newDomain }) }); this.showModal = false; this.newDomain = ''; await this.loadDomains(); } catch(e) {}
            this.saving = false;
        },
        async verifyDomain(id) { await fetch(`/api/v1/dashboard/domains/${id}/verify`, { method: 'POST', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content } }); await this.loadDomains(); },
        async deleteDomain(id) { if(!confirm('حذف هذا النطاق؟')) return; await fetch(`/api/v1/dashboard/domains/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content } }); await this.loadDomains(); }
    }
}
</script>
@endpush
