...
function customersManager() {
    return {
        ...
        async loadCustomers() {
            try {
                const res = await fetch(`/api/v1/dashboard/customers?search=${this.search}`, { headers: { 'Accept': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content } });
        ...
        exportCustomers() { window.open('/api/v1/dashboard/customers/export?format=csv', '_blank'); }
    }
}
</script>
@endpush
