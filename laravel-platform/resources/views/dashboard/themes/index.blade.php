...
function themesManager() {
    return {
        ...
        async loadThemes() {
            try {
                const r = await fetch('/api/store/themes', { headers: { 'Accept': 'application/json' } });
        ...
        async activateTheme(id) {
            try {
                await fetch(`/api/store/themes/${id}/activate`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content } });
        ...
        async saveCustomColors() {
            try {
                await fetch('/api/store/themes/customize', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content }, body: JSON.stringify({ colors: this.customColors }) });
