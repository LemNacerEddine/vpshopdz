...
function abandonedCartsManager() {
    return {
        ...
        async loadCarts() {
            try { const r = await fetch('/api/store/abandoned-carts', { headers: { 'Accept': 'application/json' } }); const d = await r.json(); this.carts = d.data || []; this.stats = d.stats || this.stats; } catch(e) {}
            this.loading = false;
        },
        ...
        async markRecovered(id) { await fetch(`/api/store/abandoned-carts/${id}/recover`, { method: 'POST', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content } }); await this.loadCarts(); }
    }
}
</script>
@endpush
