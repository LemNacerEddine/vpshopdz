...
function staffManager() {
    return {
        ...
        async loadStaff() { try { const r = await fetch('/api/v1/dashboard/staff', { headers: { 'Accept': 'application/json' } }); const d = await r.json(); this.members = d.members || []; this.invitations = d.invitations || []; } catch(e) {} },
        async sendInvitation() { await fetch('/api/v1/dashboard/staff/invite', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content }, body: JSON.stringify(this.form) }); this.showModal = false; this.resetForm(); await this.loadStaff(); },
        editMember(m) { /* TODO: edit role modal */ },
        async removeMember(id) { if(!confirm('إزالة هذا العضو؟')) return; await fetch(`/api/v1/dashboard/staff/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content } }); await this.loadStaff(); },
        async cancelInvitation(id) { await fetch(`/api/v1/dashboard/staff/invitations/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content } }); await this.loadStaff(); }
    }
}
</script>
@endpush
