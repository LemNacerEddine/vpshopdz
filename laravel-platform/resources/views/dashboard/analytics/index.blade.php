...
function analyticsManager() {
    return {
        ...
        hdrs() { return { 'Accept': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content }; },
        async loadAnalytics() {
            try {
                const [ov, tp, tw, st] = await Promise.all([
                    fetch(`/api/v1/dashboard/analytics/overview?period=${this.period}`, { headers: this.hdrs() }).then(r => r.json()),
                    fetch(`/api/v1/dashboard/analytics/top-products?period=${this.period}`, { headers: this.hdrs() }).then(r => r.json()),
                    fetch(`/api/v1/dashboard/analytics/by-wilaya?period=${this.period}`, { headers: this.hdrs() }).then(r => r.json()),
                    fetch(`/api/v1/dashboard/analytics/orders-status?period=${this.period}`, { headers: this.hdrs() }).then(r => r.json()),
                ]);
                this.stats = ov.stats || ov.data || this.stats;
                this.topProducts = tp.data || [];
                this.topWilayas = tw.data || [];
                this.renderCharts({ ...ov, status_distribution: st.data || st });
            } catch(e) { console.error(e); this.renderCharts({}); }
        },
        renderCharts(data) {
