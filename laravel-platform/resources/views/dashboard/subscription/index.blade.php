...
function subscriptionManager() {
    return {
        ...
        async loadSubscription() {
            try { const r = await fetch('/api/store/subscription', { headers: { 'Accept': 'application/json' } }); const d = await r.json(); this.current = d.current || {}; this.plans = d.plans || []; this.invoices = d.invoices || []; } catch(e) {}
        },
        async upgradePlan(plan) {
            if(!confirm(`هل تريد الترقية إلى خطة "${plan.name}" بسعر ${plan.price} د.ج/شهر؟`)) return;
            await fetch('/api/store/subscription/upgrade', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content }, body: JSON.stringify({ plan_id: plan.id }) });
