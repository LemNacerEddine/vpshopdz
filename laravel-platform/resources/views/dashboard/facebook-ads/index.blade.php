...
function facebookAdsManager() {
    return {
        ...
        async loadAds() { try { const r = await fetch('/api/store/facebook-ads', { headers: { 'Accept': 'application/json' } }); const d = await r.json(); this.ads = d.data || []; this.stats = d.stats || this.stats; } catch(e) {} this.loading = false; },
        editAd(ad) { this.editMode = true; this.editId = ad.id; this.form = {...ad}; this.showModal = true; },
        async saveAd() {
            const url = this.editMode ? `/api/store/facebook-ads/${this.editId}` : '/api/store/facebook-ads';
        ...
        async deleteAd(id) { if(!confirm('حذف هذه الحملة؟')) return; await fetch(`/api/store/facebook-ads/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content } }); await this.loadAds(); }
    }
}
</script>
@endpush
