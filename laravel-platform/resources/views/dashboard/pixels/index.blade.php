...
function pixelsManager() {
    return {
        ...
        async loadPixels() { try { const r = await fetch('/api/v1/dashboard/pixels', { headers: { 'Accept': 'application/json' } }); this.pixels = (await r.json()).data || []; } catch(e) {} this.loading = false; },
        async savePixel() { await fetch('/api/v1/dashboard/pixels', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content }, body: JSON.stringify(this.form) }); this.showModal = false; this.resetForm(); await this.loadPixels(); },
        async togglePixel(pixel) { await fetch(`/api/v1/dashboard/pixels/${pixel.id}/toggle`, { method: 'PUT', headers: { 'Accept': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content } }); await this.loadPixels(); },
        async deletePixel(id) { if(!confirm('حذف هذا البكسل؟')) return; await fetch(`/api/v1/dashboard/pixels/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content } }); await this.loadPixels(); }
    }
}
</script>
@endpush
