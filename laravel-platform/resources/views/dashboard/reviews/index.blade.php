...
function reviewsManager() {
    return {
        ...
        hdrs() { return { 'Accept': 'application/json', 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content }; },
        async loadReviews() {
            try { const r = await fetch(`/api/v1/dashboard/reviews?status=${this.filter}`, { headers: this.hdrs() }); const d = await r.json(); this.reviews = d.data || []; this.stats = d.stats || this.stats; } catch(e) {}
            this.loading = false;
        },
        async updateStatus(id, status) {
            const endpoint = status === 'approved' ? 'approve' : 'approve';
            await fetch(`/api/v1/dashboard/reviews/${id}/${endpoint}`, { method: 'PUT', headers: this.hdrs(), body: JSON.stringify({ status }) });
            await this.loadReviews();
        },
        openReply(review) { this.replyReviewId = review.id; this.replyText = review.reply || ''; this.showReplyModal = true; },
        async submitReply() { await fetch(`/api/v1/dashboard/reviews/${this.replyReviewId}/reply`, { method: 'PUT', headers: this.hdrs(), body: JSON.stringify({ reply: this.replyText }) }); this.showReplyModal = false; await this.loadReviews(); },
        async deleteReview(id) { if(!confirm('حذف هذا التقييم؟')) return; await fetch(`/api/v1/dashboard/reviews/${id}`, { method: 'DELETE', headers: this.hdrs() }); await this.loadReviews(); }
    }
}
</script>
@endpush
