...
function mediaManager() {
    return {
        ...
        async loadMedia() { try { const r = await fetch('/api/v1/dashboard/media', { headers: { 'Accept': 'application/json' } }); const d = await r.json(); this.media = d.data || []; this.storageUsed = d.storage_used || '0 MB'; this.storageLimit = d.storage_limit || '500 MB'; } catch(e) {} this.loading = false; },
        async uploadFiles(event) {
            ...
            xhr.open('POST', '/api/v1/dashboard/media/upload');
            xhr.setRequestHeader('X-CSRF-TOKEN', document.querySelector('meta[name="csrf-token"]').content);
            xhr.send(formData);
        },
        copyUrl(file) { navigator.clipboard.writeText(file.url); },
        selectFile(file) { /* TODO: preview modal */ },
        async deleteFile(id) { if(!confirm('حذف هذا الملف؟')) return; await fetch(`/api/v1/dashboard/media/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content } }); await this.loadMedia(); }
    }
}
</script>
@endpush
