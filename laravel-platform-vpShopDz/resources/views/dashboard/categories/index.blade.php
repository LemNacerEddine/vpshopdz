...
function categoriesManager() {
    return {
        ...
        async loadCategories() {
            try {
                const res = await fetch('/api/v1/dashboard/categories', { headers: { 'Accept': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content } });
        ...
        async saveCategory() {
            this.saving = true;
            try {
                const url = this.editMode ? `/api/v1/dashboard/categories/${this.editId}` : '/api/v1/dashboard/categories';
        ...
        async deleteCategory(id) {
            ...
            try {
                await fetch(`/api/v1/dashboard/categories/${id}`, {
