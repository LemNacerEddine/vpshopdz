...
function notificationsManager() {
    return {
        ...
        async loadSettings() {
            try { const r = await fetch('/api/v1/dashboard/notifications/settings', { headers: this.hdrs() }); const d = await r.json(); this.settings = {...this.settings, ...(d.data || d)}; } catch(e) {}
        },
        async saveSettings() {
            await fetch('/api/v1/dashboard/notifications/settings', { method: 'PUT', headers: this.hdrs(), body: JSON.stringify(this.settings) });
        },
        async testTelegram() {
            const r = await fetch('/api/v1/dashboard/notifications/test', { method: 'POST', headers: this.hdrs(), body: JSON.stringify({ channel: 'telegram' }) });
