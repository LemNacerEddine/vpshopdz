...
class Order {
    ...
    // Generate yearly order ID: ORD-XXXX/YYYY (atomic, safe for concurrent requests)
    private function generateOrderId() {
        $year = (int)date('Y');

        // Atomic increment: INSERT or UPDATE in one query, no race condition
        $query = "INSERT INTO order_sequence (year, last_number) VALUES (:year, 1)
                  ON DUPLICATE KEY UPDATE last_number = last_number + 1";
        $stmt = $this->conn->prepare($query);
        $stmt->execute([':year' => $year]);

        // Read the assigned number
        $query = "SELECT last_number FROM order_sequence WHERE year = :year";
        $stmt = $this->conn->prepare($query);
        $stmt->execute([':year' => $year]);
    ...
    public function create($data) {
        ...
        $query = "INSERT INTO {$this->table}
                  (order_id, user_id, customer_name, customer_phone, customer_email,
                   shipping_address, wilaya, commune, subtotal, shipping_cost, discount_amount, discount_percentage, total, payment_method, notes, status)
                  VALUES
                  (:order_id, :user_id, :customer_name, :customer_phone, :customer_email,
                   :shipping_address, :wilaya, :commune, :subtotal, :shipping_cost, :discount_amount, :discount_percentage, :total, :payment_method, :notes, 'pending')";
